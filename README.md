...
`ours-cowork` is a standalone standard-SDK daemon for durable ours mission rooms. It owns one ordinary ours identity per room through an embedded `@ours.network/sdk` runtime, an ordered local archive, a private Unix management socket, and a localhost web console with room RPC.
