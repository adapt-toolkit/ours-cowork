...
function parseOptions(
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (allowedBooleans.has(token)) {
      if (booleans.has(token)) usageError(`duplicate option ${token}`);
      booleans.add(token);
      continue;
    }
    if (!allowedValues.has(token)) usageError(`unknown or not allowed option ${token}`);
    if (Object.hasOwn(values, token)) usageError(`duplicate option ${token}`);
...
function rpcCall(socketPath: string, method: string, params: Record<string, unknown>): Promise<unknown> {
  ...
  return new Promise((resolveCall, rejectCall) => {
    ...
    const timer = setTimeout(() => finishError(new CliError(EXIT.daemonUnavailable, 'daemon_unavailable', 'daemon did not answer the management socket')), RPC_TIMEOUT_MS);
...
function runningPid(config: CoworkConfig): number | null {
...
async function waitForSocket(path: string, timeoutMs: number): Promise<boolean> {
  ...
  while (Date.now() < deadline) {
    if (await socketOpen(path)) return true;
    await sleep(200);
  }
  return false;
}
...
async function startDaemon(config: CoworkConfig): Promise<{ pid: number; alreadyRunning: boolean }> {
  ...
  const existing = runningPid(config);
  if (existing !== null && await socketOpen(runtime.socketPath)) return { pid: existing, alreadyRunning: true };
  ...
  if (!await waitForSocket(runtime.socketPath, START_TIMEOUT_MS)) {
    throw new CliError(EXIT.internal, 'internal', `daemon did not become ready; inspect ${logPath}`);
  }
  return { pid: runningPid(config) ?? child.pid, alreadyRunning: false };
}
...
function installSystemd(config: CoworkConfig): string {
  ...
  const environment = Object.entries(serviceEnvironment(config))
    .map(([key, value]) => `Environment=${key}=${value.replaceAll('\\', '\\\\').replaceAll(' ', '\\x20')}`)
    .join('\n');
  const unit = `[Unit]
...
function uninstallSystemd(): string {
  runTool('systemctl', ['--user', 'disable', '--now', SYSTEMD_UNIT]);
  const path = systemdUnitPath();
...
async function execute(args: string[], output: Output): Promise<void> {
  ...
  switch (command) {
    case 'serve': {
      ...
      const daemon = await import(modulePath) as { runSupervisor: () => Promise<number> };
      const code = await daemon.runSupervisor();
    ...
    case 'status': {
      ...
      const pid = runningPid(config);
      if (pid === null || !await socketOpen(runtime.socketPath)) {
...
async function main(): Promise<void> {
  ...
  const jsonCount = raw.filter((argument) => argument === '--json').length;
  if (jsonCount > 1) {
