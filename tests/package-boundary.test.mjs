...
test('package stays an independent cowork daemon', async () => {
  ...
  assert.equal(dependencies['@ours.network/sdk'], '3.8.1-nightly.6');
  assert.equal(pkg.devDependencies['@ours.network/cli'], '2.8.1-nightly.4');
  ...
  assert.equal(sdkVersion, '3.8.1-nightly.6');
  assert.equal(cliVersion, '2.8.1-nightly.4');
  const publicDocs = {
